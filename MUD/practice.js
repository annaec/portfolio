...
function engage() {
	if (currentLocation == "s_gate") {
			...
			$("<p>The southwestern corner of this realm is known for its attention-seeking, and sometimes vengeful spirits. A piercing wail strikes the wind and your heart seizes. A shadow whips around you. Legend has it that the spirits in this realm are afraid of nothing but their own reflection. With stiff limbs you reach into your satchel praying to grasp a mirror.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "s_east") {
			$("<p>The southeastern corner is home to the humans of this realm. You hear the sounds of industrious life all around you, and long for the family you left behind.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "castle"){
			$("<p>The Castle is more brilliant than you could have imagined. The entrance is elaborately carved with the curiosities native to this realm - Spirits, ogres, and treasures in great bounty. More than anything, you wish to enter, but the door is locked.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "west") {
			$("<p>The day is at its end and the western realm is dark. Your surroundings are obscured by the setting sun, leaving you vulnerable to creatures unknown.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "east") {
			$("<p>The eastern sunrise is exquisite, but your hunger is consuming you.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "n_west") {
			$("<p>The northwestern corner of this realm is marked with bare treasure coves, all raided long ago by creatures seeking great riches and power. Ogres have made this their home, hostile beasts motivated by hunger and greed. The ground shakes beneath you, and you know an ogre has caught your scent. You have three choices: Try to strike him with your sword, pierce his eyes with your dagger, or buy his mercy with your ruby...</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "n_gate") {
			$("<p>You have strayed far from your entry into this realm, and you feel your journey coming to an end. Your thirst is as acute as your desire for a long-awaited safe haven, and you search around your satchel for something to drink.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else if (currentLocation == "n_east") {
			$("<p>The northeastern corner of this realm is life-threateningly cold. You've heard tales of the dead trapped here, eternally searching for warmth, frozen from within. One such ice creature is making its way towards you in the distance. You have time for just one action: wrap yourself in a warm layer to save yourself from a frozen fate, or brandish your sword against the enemy.</p>")
		}
}
...
function pickUpSword() {
	if (currentLocation == "s_gate") {
			...
			updateInventory();
			$("<p>You picked up the sword. The weight of the blade feels powerful, and you are emboldened to explore the world around you. Move NORTH, SOUTH, EAST, or WEST.</p>").insertBefore("#placeholder").fadeIn(1000);
		} else {
...
function pickUpDagger() {
	if(currentLocation == "s_east") {
		...
		$("<p>The dagger is not here. Proceed to the southeast for a pocket weapon, or continue to explore.</p>").insertBefore("#placeholder");
	}
}
...
function pickUpBread() {
	if(currentLocation == "east") {
		...
		updateInventory();
		$("<p>You picked up the bread, consumed by hunger.</p>").hide().insertBefore("#placeholder").fadeIn(1000);
	} else {
...
function useSword() {
	...
	$("<p>You wield your sword against your fearsome enemy. The blade strikes him and he slows, but does not die. Run!</p>").insertBefore("#placeholder").fadeIn(1000);
}
...
function useCloak() {
	...
	updateInventory();
	$("<p>You pull the cloak from your satchel and wrap it around your body. You can instantly feel your strength return.</p>").insertBefore("#placeholder").fadeIn(1000);
}

function useKey() {
	removeItem(newPlayer.inventory, "Key");
	updateInventory();
	$("<p>You grasp the heavy key in both hands and turn it in the expertly crafted Castle lock. It clicks open, you enter. A castle guard is stationed directly inside, poised for battle. You have three choices, run, bribe, or fight.</p>").insertBefore("#placeholder").fadeIn(1000);
}

function useDagger() {
	removeItem(newPlayer.inventory, "Dagger");
	updateInventory();
	$("<p>You grip the dagger and bury it into your enemy's good eye. He bellows in pain, and crumbles to the ground, clutching his face. You have time to escape!</p>").insertBefore("#placeholder").fadeIn(1000);
}
...
function useWine() {
	...
	updateInventory();
	$("<p>You gulp your wine with great fervor. Continue to explore.</p>").insertBefore("#placeholder").fadeIn(1000);
}

function useMirror() {
	removeItem(newPlayer.inventory, "Mirror");
	updateInventory();
	$("<p>You pull the mirror from your satchel and thrust it towards the spirit. It shrieks at the sight of its own reflection and retreats, leaving a thin veil of soot in its path.</p>").insertBefore("#placeholder").fadeIn(1000);
}
...
$(document).ready(function () {
  ...
  // 		$("#inventory li").remove();
		...
		var arriveNorthwest = "<p>You are now at the northwestern corner of this world. You've heard legends of treasures in this realm, and peel your eyes for riches among the muted colors of the earth around you. Against all odds, a faint red glow is flickering against a tree root nearby. A ruby!</p>";
		arriveNorthGate = "<p>You are now at the northern gate of the Castle. Your journey has made you thirsty. A wine cellar extends beyond the northern Castle grounds, and you see an unattended wagon stacked with small flasks of wine.</p>";
		arriveNortheast = "<p>You are now at northeastern corner of this world. There is a chill in the north, and you see a cast-off cloak lining the road.</p>";
		...
		arriveCastle = "<p>You are now at the foot of the Castle. You feel a fleeting pull at your hem and turn around to find someone has left an intricate key at your heels.</p>";
		arriveEast = "<p>You are now at the eastern most territory of this world. The sun is rising, and you feel the faint ache of hunger. A baker in the East Market is giving away last week's bread, and the stale yeasty scent is drawing you in.</p>";
		arriveSouthwest = "<p>You are now at the southwestern corner of this world. The sun catches the surface of a small, ornamented mirror on your path.</p>";
		...
		arriveSoutheast = "<p>You are now at the southeastern corner of this world. In a clearing you see a young girl pantomiming a knight in battle, impossibly light on her feet. She is clutching a small hunting dagger in her hand, which she drops and kicks away at the sound of her mother's voice calling her home.</p>";
